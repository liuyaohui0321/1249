...
//		xil_printf("%s %d\r\n", __FUNCTION__, __LINE__);
		...
		//可以增加标志位，判断此时应该以网口方式上传还是gth_dma（根据过来的中断是网口receive还是gth_dma）
//		if(lwip_flag==1)
//		{
//			TcpProcessTransmission((UINTPTR)&ReplyStructA203Ack,sizeof(StructA203Ack));
//			lwip_flag=0;
//		}
...
//					break;
//				}
				if(cmd_write_cnt==51)		// 3.19号改 by lyh
				{
					xil_printf("I/O Write Finish!\r\n");
//					xil_printf("w_count = %u\r\n",cmd_write_cnt);
					xil_printf("w_count = %u w_size = %lu\r\n",cmd_write_cnt,f_size(&wfile));
...
//		TcpProcessTransmission((UINTPTR)&SendStructHealthStatus,sizeof(StructHealthStatus));
		return 0;
}
